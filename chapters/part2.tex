...
\begin{definition}[differentiable]%
    ...
    \[
        f(\xx) = f(\aa) + {df}_{\aa}(\xx-\aa) + \epsilon(\aa, \xx - \aa)
    \]
    where \(\abs{\epsilon(\aa, \xx - \aa)} = \littleo{\norm{\xx-\aa}}\).
\end{definition}
...
\begin{theorem}%
    ...
    \[
        f(\xx) = f(\aa) +   \nabla f(\aa) \cdot (\xx-\aa) + \epsilon(\aa, \xx-\aa)
    \]
    where \(\abs{\epsilon(\aa, \xx - \aa)} = \littleo{\norm{\xx-\aa}}\).
...
\begin{proof}
    ...
    \(  f(\aa+ h \vv) = f(\aa) + h {df}_{\aa}(\vv) + \epsilon(\aa, h\vv)\)
    and hence
    \[
        \begin{aligned}
            D_{\vv}f(\aa) & =
            \lim_{h\to 0} \frac{1}{h}(f(\aa+h \vv) - f(\aa)) \\
                          & =
            \lim_{h\to 0} \frac{1}{h}(h \ {df}_{\aa}(\vv) + \epsilon(\aa, h\vv) )
            =  {df}_{\aa}(\vv).
        \end{aligned}
    \]
...
\end{theorem*}
\begin{proof}
    Observe that \(\abs{f(\aa+\vv)-f(\aa)} = \abs{   {df}_{\aa}(\vv) + \epsilon(\aa, \vv)  }\).
    This means that 
    \[
        \abs{f(\aa+\vv)-f(\aa)} \leq \norm{  {df}_{\aa} } \norm{\vv} + \abs{\epsilon(\aa, \vv)} 
    \] 
...
\begin{proof}
    ...
    we have
    \[
     f(\xx) = f(\aalpha(t)) + \nabla f(\aalpha(t)) \cdot (\aalpha(t+h) - \aalpha(t)) + \epsilon(\aalpha(t), \aalpha(t+h)-\aalpha(t)).
    \]
    Then
    \[
        \begin{aligned}
            \tfrac{1}{h}\left[g(t+h)-g(t) \right]
             ...
             & \ \ +  \tfrac{1}{h}  \epsilon(\aalpha(t), \aalpha(t+h)-\aalpha(t)).
        \end{aligned}
    \]
    Observe that \(\tfrac{1}{h}  (\aalpha(t+h)-\aalpha(t)) \to \aalpha'(t)  \) as \(h\to 0\)
    and $\tfrac{1}{h}  \epsilon(\aalpha(t), \aalpha(t+h)-\aalpha(t)) \to 0$
    as in the case of one-dimensional functions
    (we can write $\epsilon(\aalpha(t), \vv) = \norm{\vv}E(\aalpha(t), \vv)$,
    where $E(\aalpha(t), \vv) \to 0$ as $\vv \to 0$ and we set $E(\aalpha(t), 0) = 0$).
    .
\end{proof}
...
\begin{definition*}[differentiable]
    ...
    We say that \(\ff: S \to \bR^m\) is \emph{differentiable} at \(\aa\) if there exists a linear transformation \({df}_{\aa}: \bR^n \to \bR^m\) such that, for \(\xx \in B(\aa,r)\),
    \[
        \ff(\xx) = \ff(\aa) + {df}_{\aa}(\xx-\aa) + \eepsilon(\aa, \xx-\aa)
    \]
    \(\norm{\eepsilon(\aa, \xx-\aa)} = \littleo{\norm{\xx-\aa}}\).
\end{definition*}
...
\[
    \ff(\xx) = \ff(\aa) +  D\ff(\aa) (\xx-\aa) + \eepsilon(\aa, \xx-\aa)
\]
where \(\norm{\eepsilon(\aa, \xx-\aa)} = \littleo{\norm{\xx-\aa}}\).
...
\begin{example*}[polar coordinates]
    ...
    \]
    as the function  \(\ff(r,\theta) = (x(r,\theta),y(r,\theta))\) where \(\ff:(0,\infty)\times [0,2\pi) \to \bR^2\). % chktex 9
...
\section{Implicit functions \& partial derivatives}
...
\begin{center}
    \begin{tabular}{c | c} % chktex 44
        ...
        \(x^2+y^2+z^2=1\)           & \(y(x) = \pm \sqrt{1-x^2-y^2}\), \(\sqrt{x^2+y^2}\geq 1\) \\
        \(x^2+y^2+yz -e^y -4 =0\)     & A mess?                                        \\
        \(x^2y^4 - 3 = \sin(xyz)\) & A huge mess?
    \end{tabular}
\end{center}

Given the above observation, the following method of calculating derivatives is sometimes useful.
Suppose that some \(F:\bR^3 \to \bR\) is given and we suppose there exists some \(z:\bR^2\to\bR\) such that
\[
    F(x,y, z(x,y))=0 \quad \text{ for all \(x,y\)}.
\]
Let \(h(x,y):= F(x,y,z(x))\) and note that \(D_x h(x,y) = D_y h(x,y) = 0\).
Here we are using the idea that \(h = F \circ \gg\) where \(\gg(x,y) = (x,y,z(x,y))\).
By the chain rule,
\begin{align*}
 0 &= \frac{\partial h}{\partial x}(x,y) \\
 &= \frac{\partial F}{\partial x}(x,y,z(x,y))\cdot 1 +
 \frac{\partial F}{\partial y}(x,y,z(x,y))\cdot 0 \\
 &\quad +  \frac{\partial F}{\partial z}(x,y,z(x,y))\cdot \frac{\partial z}{\partial x}(x,y),
\end{align*}
therefore,
\[
 \frac{\partial z}{\partial x}(x,y) =
 -\frac{\frac{\partial F}{\partial x}(x,y,f(x,y))}{\frac{\partial F}{\partial z}(x,y,f(x,y))}.
\]
Similarly,
\[
 \frac{\partial z}{\partial y}(x,y) =
 -\frac{\frac{\partial F}{\partial y}(x,y,f(x,y))}{\frac{\partial F}{\partial z}(x,y,f(x,y))}.
\]
Note that $F(x,y,z)$ is \textbf{explicitly given}.


\begin{example}
 $ F(x,y,z) = x^2 + y^2 + z^2 - 1$.
 There is $z(x,y) = \sqrt{1-x^2-y^2}$ such that $F(x,y,z(x,y)) = 0$.
 Then $\frac{\partial z}{\partial x} = -\frac{\frac{\partial F}{\partial x}}{\frac{\partial F}{\partial z}} = -\frac{2x}{2z}$.
 Using $z = \sqrt{1-x^2-y^2}$, we get $\frac{\partial z}{\partial x} = -\frac{x}{\sqrt{1-x^2-y^2}}$.
 
 \noindent
 \textbf{Solution.} $F(0,e,f(0,e)) = F(0,e,2) = e^2 + 0 + 2^2 - e^2 - C = 0 \Longrightarrow C = 4$.
 Note that
 \begin{itemize}
  \item $\frac{\partial F}{\partial x}(x,y,z) = z$ and hence $\frac{\partial F}{\partial x}(x,y,f(x,y)) = f(x,y)$
  \item $\frac{\partial F}{\partial y}(x,y,z) = 2y$ and hence $\frac{\partial F}{\partial y}(x,y,f(x,y)) = 2y$
  \item $\frac{\partial F}{\partial z}(x,y,z) = x + 2z - e^z$ and hence $\frac{\partial F}{\partial z}(x,y,f(x,y)) = x + f(x,y) - e^{f(x,y)}$
 \end{itemize}
 Therefore, 
 \begin{align*}
 \frac{\partial f}{\partial x}(x,y) &=
 -\frac{\frac{\partial F}{\partial x}(x,y,f(x,y))}{\frac{\partial F}{\partial z}(x,y,f(x,y))}  = -\frac{f(x,y)}{x+2f(x,y)-e^{f(x,y)}}\\
 \frac{\partial f}{\partial x}(0,e) &=
-\frac{f(0,e)}{0+2f(0,e)-e^{f(0,e)}}  = \frac{2}{e^2 - 4}.\\ 
 \frac{\partial f}{\partial y}(x,y) &=
 -\frac{\frac{\partial F}{\partial y}(x,y,f(x,y))}{\frac{\partial F}{\partial z}(x,y,f(x,y))}  = -\frac{2y}{x+2f(x,y)-e^{f(x,y)}}\\
 \frac{\partial f}{\partial y}(0,e) &=
-\frac{2e}{0+2f(0,e)-e^{f(0,e)}}  = \frac{2e}{e^2 - 4}.\\ 
\end{align*}

\end{example}

\begin{example}
 $ F(x,y,z) = y^2 + xz + z^2 - e^z - C$, where $C\in\bR$.
 Assume the existence of $f(x,y)$ such that $F(x,y,f(x,y)) = 0$.
 Find the value of $C$ such that $f(0,e) = 2$ and compute $\frac{\partial f}{\partial x}(0,e), \frac{\partial f}{\partial y}(0,e)$.
...
\end{example}
More generally, if $F(x_1,\cdots, x_n) = 0$ defines a function $x_n = f(x_1,\cdots, x_{n-1})$, then
\[
 \frac{\partial f}{\partial x_k}(x_1,\cdots, x_{n-1}) =
 -\frac{\frac{\partial F}{\partial x_k}(x_1,\cdots, x_{n-1}, f(x_1,\cdots, x_{n-1}))}{\frac{\partial F}{\partial x_n}(x_1,\cdots, x_{n-1}, f(x_1,\cdots, x_{n-1}))}.
\]
...
% \begin{itemize}
%     \item Apostol, ``Calculus Volume 2'', Chapter 8
% \end{itemize}
