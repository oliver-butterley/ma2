...
\section{Extrema and other applications}
...
% \begin{task}
%     Modeling one smaller body in orbit around another larger body we obtain the potential describing the combination of gravitational and centripetal force,
%     \[
%         \Psi(x,y)
%         = (x - \tfrac{1}{11})^2 + y^2 + \tfrac{20}{11}\left(x^2+y^2\right)^{-\frac{1}{2}} + \tfrac{2}{11} \left( (x-1)^2 + y^2 \right)^{-\frac{1}{2}}.
%     \]
%     Investigate as follows:
%     \begin{enumerate}
%         \item What are the poles of \(\Psi\)? These correspond to the centres of the bodies.
%         \item Calculate the gradient \(\nabla \Psi\).
%         \item Suppose that \(y=0\) and search for stationary points. After reducing to an equation in \(x\) use a numerical solving tool.        
%         \item Suppose that \(x^2 + y^2 = 1\) and search for stationary points.
%     \end{enumerate} 
% \end{task}