...
For convenience, in components we write \(\aalpha(t) = (\alpha_1(t),\ldots,\alpha_n(t))\).
We say that \(\aalpha(t)\) is \emph{differentiable} if each component \(\alpha_k(t)\) is differentiable on \([a,b]\) and \(\alpha_k'(t)\) is continuous (Definition~\ref{def:differentiable}).
We say that \(\aalpha(t)\) is \emph{piecewise differentiable} if \([a,b] = [a,c_1]\cup[c_1,c_2] \cup \cdots \cup [c_l,b]\) and \(\aalpha(t)\) is \emph{differentiable} on each of these intervals.

\begin{definition}%
    \label{def:path}
    If \(\aalpha: [a,b] \to \bR^n\) is piecewise differentiable then we call it a \emph{path}.
\end{definition}
...
Let \(\aalpha(t)\) be a (piecewise differentiable) path on \([a,b]\) and
...
\begin{definition}[line integral of a vector field]%
    \label{def:line-integral-vector}
...
Each of these different notations are in common usage in different contexts but the underlying quantity is always the same.
...
\begin{definition}[equivalent paths]
    We say that two paths \(\aalpha(t)\) and \(\bbeta(t)\) are \emph{equivalent} if there exists a differentiable function \(u : [c,d] \to [a,b] \) such that \(\aalpha(u(t)) = \bbeta(t)\).
...
This is called the \href{https://en.wikipedia.org/wiki/Fundamental_theorem_of_calculus#Second_part}{second fundamental theorem of calculus} and is one of the ways in which we see that differentiation and integration are opposites.
...
\begin{proof}
    Suppose that \(\aalpha(t)\) is differentiable.
...
Recall that, if \(f:\bR \to \bR\) is continuous and we let \(\varphi(x) = \int_a^x f(t) \ dt\) then \(\varphi'(x) = f(x)\).
This is called the \href{https://en.wikipedia.org/wiki/Fundamental_theorem_of_calculus#First_part}{first fundamental theorem of calculus} and is the other way in which we see that differentiation and integration are opposites.
Again we have an analog for the line integral but here it becomes a little more subtle since there are many different paths along which we can integrate between any two points.
...
\begin{theorem}[conservative vector fields]
    Let \(S \subset \bR^n\) and and consider the vector field \(\ff : S \to \bR^n\).
    The following are equivalent:
    \begin{enumerate}[label=\textnormal{(\roman*)}]
        \item \(\ff\) is conservative, i.e., \(\ff = \nabla \varphi\) on \(S\) for some \(\varphi\),
        \item \(\int \ff \cdot d\aalpha\) does not depend on \(\aalpha\), as long as \(\aalpha(a)=\aa\), \(\aalpha(b)=\bb\),
        \item \(\int \ff \cdot d\aalpha = 0\) for any closed path \(\aalpha\) contained in \(S\).
    \end{enumerate}
\end{theorem}
...
\begin{theorem}[mixed derivatives in 2D]%
    \label{thm:mixed-partials-2D}
...
%
\begin{theorem}[mixed derivatives]%
    \label{thm:mixed-partials}
    Suppose that \(\ff\) is a differentiable vector field\footnote{As before  \(f_k(x_1,\ldots,x_n)\) denotes the \(k\)\textsuperscript{th} component of the vector field \(\ff\).} on \(S\subset \bR^n\).

    If \(\ff\) is conservative then, for each \(l,k\),
    \[
...
\section{Potentials \& conservative vector fields}
...
\begin{theorem}
    Let\footnote{As usual  \(  f_k(x_1,\ldots,x_n)\) denotes the \(k\)\textsuperscript{th} component of the vector field \(\ff\).} \(\ff\) be a differentiable vector field on a convex region \(S\subset \bR^n\).
...
Using the idea of ``gluing together'' several convex regions this result can be extended to a more general setting.
For this we require the notion of simply-connected sets as follows.
...
\begin{figure}
    ...
    \caption{A set which fails to be simply-connected because paths like the one shown cannot be contracted to a point.}
\end{figure}

\subsection*{Application to exact differential equations}

Let \(S\subset \bR^2\) be simply-connected and open.
The differential equation, considered on \(S\),
\[
    p(x,y) + q(x,y) y'(x) = 0
\]
is called \emph{exact} if there exists \(\varphi : S \to \bR\) such that
\[
    p = \frac{\partial \varphi}{\partial x},
    \quad
    q = \frac{\partial \varphi}{\partial y}.
\]
\href{https://en.wikipedia.org/wiki/Exact_differential_equation}{Exact differential equations} are closely related to conservative vector fields.
% 
\begin{theorem}%
    \label{thm:exact-diff-eq}
    Let \(S\subset \bR^2\) be simply-connected and open.
    \begin{itemize}
        \item     Suppose that \(\varphi: S \to \bR\) satisfies
              \(\nabla \varphi =   \left(\begin{smallmatrix}
                  p\\ q
              \end{smallmatrix}\right)\).
              Then the solution \(y(x)\) of the equation \(p(x,y) = q(x,y) y'(x)\) satisfies \(\varphi(x,y(x))=C\) for some \(C\in \bR\).
        \item
              Conversely, if \(\varphi: S \to \bR\) is such that \(\varphi(x,y(x))=C\)  defines implicitly a function \(y(x)\), then \(y(x)\) is a solution to the equation  \(p(x,y) = q(x,y) y'(x)\).
    \end{itemize}
\end{theorem}
...
\begin{proof}
    If \(y(x)\) satisfies  \(\varphi(x,y(x))=C\), then by the chain rule and the fact that \(\nabla \varphi =   \left(\begin{smallmatrix}
        p \\ q
    \end{smallmatrix}\right)\), 
    we see that \(p(x,y(x)) + y'(x) q(x,y(x)) = 0\).
...
Up until now this chapter has been devoted to line integrals of vector fields but there is also the obvious question of defining the line integral for scalar fields.
This we do now.
Such a line integral allows us also to define the \emph{length of a curve} in a meaningful way.
Let \(\aalpha(t)\), \(t\in [a,b]\) be a path in \(\bR^n\) and let \(f:\bR^n \to \bR\).

\begin{definition}[line integral of a scalar field]%
    \label{def:line-integral-scalar}
    The \emph{line integral} of the scalar field \(f\) along the path \(\aalpha\) is defined as
    \[
        \int f \ d\alpha = \int_a^b f(\alpha(t)) \ \norm{\aalpha'(t)} \ dt.
    \]
\end{definition}

This integral shares the same basic properties of the line integral of a vector field and the proofs are essentially the same.
Namely it is linear and also respects how a path can be decomposed or joined with other paths which changing the value of the integral.
Moreover, the value of the integral along a given path is independent of the choice of parametrization of the curve.
In this case, even if the curve is parametrized in the opposite direction then the integral takes the same value.
Consequently it makes sense to define the length of the curve as the line integral of the unit scalar field, i.e., the length of a curve parametrized by the path \(\aalpha\) is \( \int_a^b \norm{\aalpha'(t)} \ dt\).

As a simple application, consider that the path represents a wire and the wire has density \(f(\aalpha(t))\) at the point \(\aalpha(t)\).
Then the mass of the wire is equal to \(\int  f \ d\alpha\).