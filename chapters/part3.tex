...
\begin{theorem}[Lagrange multipliers in 2D]
    Suppose that \(f(x,y)\), \(g(x,y)\) are differentiable scalar fields.
    Let \(G = \{(x,y) : g(x,y) = 0\}\).
    Suppose that \((x_0,y_0) \in G\) is a extrema point for \(f\) constrained to \(G\). 
    Then there exists a unique scalar \(\lambda\) such that,
    \[
...
\begin{theorem}[Lagrange multipliers in 3D]
    ...
    For each extremum point, there exist scalars \(\lambda_1\), \(\lambda_2\) such that, at that point
    \[
        \nabla f = \lambda_{1} \nabla g_{1} + \lambda_{2} \nabla g_{2}.
    \]
\end{theorem}
...
\begin{theorem*}[Lagrange multipliers]
    Suppose that a differentiable scalar field \(f(x_1,\ldots,x_n)\) has a relative extrema when it is subject to \(m\) constraints
    \[
        g_1(x_1,\ldots,x_n) = 0,
        \dots , g_m(x_1,\ldots,x_n)=0,
    \]
    where \(m < n\), and the \(\nabla g_k\) are all linearly independent.
    Then, for each extremum point, there exist \(m\) scalars \(\lambda_1,\ldots,\lambda_m\) such that, at that point,
    \[
