...
If \(f:\bR \to \bR\) is differentiable, then, if \(x\) is close to \(a\),
\[
    f(x) \approx f(a) + (x-a) f'(a) 
\]
in the sense that\footnote{This is \href{https://en.wikipedia.org/wiki/Big_O_notation\#Little-o_notation}{\emph{little-o notation}} and here means that \(\abs{f(x) - f(a) - (x-a) f'(a)}/\abs{x-a} \to 0\) as \(\abs{x-a}\to 0\).  } \(\abs{f(x) - f(a) - (x-a) f'(a)} = \littleo{\abs{x-a}}\).

We will use this idea in the higher dimensional definition of differentiability.
...
\begin{definition*}[differentiable]
    ...
    \[
        f(\xx) \approx f(\aa) + {df}_{\aa}(\xx-\aa)
    \]
    in the sense that \(\abs{f(\xx) - f(\aa) - {df}_{\aa}(\xx-\aa)} = \littleo{\norm{\xx-\aa}}\).
\end{definition*}
...
\begin{theorem}
    ...
    \]
    in the sense that \(\abs{f(\xx) - f(\aa) - \nabla f(\aa) (\xx-\aa)} = \littleo{\norm{\xx-\aa}}\).
    Moreover 
    \[
        D_{\vv}f(\aa) = \nabla f(\aa) \cdot \vv.  
    \]
